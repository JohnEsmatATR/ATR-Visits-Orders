...
import com.akhnaton.foodvisits.domin.Promoter2Repository
        ...
        viewModelScope.launch {
            promoterIntent.consumeAsFlow().collect {
                when (it) {
                    ...
                    is PromoterIntent.CheckIn -> checkInGPS(
                        it.checkInGPSReq,
                    )

                    is PromoterIntent.PromoterGetItemData -> promoterGetItemData(
                        it.customerCode,
                        it.partySiteId
                    )

                    is PromoterIntent.PromoterSaveStock -> promoterSaveStock(
    ...
    private fun checkInGPS(checkInGPSReq: CheckInGPSReq) {
        ...
        viewModelScope.launch {
            ...
            _status.value = try {
                ...
                PromoterStatus.CheckIn(
                    Promoter2Repository().checkInGPS(checkInGPSReq)
                )
            } catch (e: Exception) {
    ...
    private fun promoterGetItemData(customerCode: String, partySiteId: String) {
        ...
        viewModelScope.launch {
            ...
            _status.value = try {
                ...
                PromoterStatus.PromoterGetItemData(
                    Promoter2Repository().promoterGetItemData(customerCode, partySiteId)
                )
            } catch (e: Exception) {
    ...
    private fun promoterSaveStock(promoterSaveStockReq: PromoterSaveStockReq) {
        ...
        viewModelScope.launch {
            ...
            _status.value = try {
                ...
                PromoterStatus.PromoterSaveStock(
                    Promoter2Repository().promoterSaveStock(promoterSaveStockReq)
                )
            } catch (e: Exception) {
