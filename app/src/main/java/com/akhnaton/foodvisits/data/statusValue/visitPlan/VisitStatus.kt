...
sealed class VisitStatus {
    ...
    object LoadingMore : VisitStatus()
    data class GetMonthlyVisits(val response: VisitListModel) : VisitStatus()
    data class GetPendingVisits(val response: PendingVisitsRes, val isLoadMore: Boolean) : VisitStatus()
    data class ApproveVisits(val response: ApproveVisitsRes) : VisitStatus()
